...
def main(base_path: Path, max_edges: int | None = 60_000) -> int:
    ...
    for hz in HAZARDS:
        ...
        # NIRD_RESULTS_VARIANT from the environment (default "revision") --
        # NOT a parameter it takes. Set it explicitly per hazard rather than
        # relying on ambient shell state: confirmed this was the second half
        # of the n=0-damaged bug (the first half was road_links_path()'s own
        # stale damage_level_max column) -- without RESIFLOW_RESULTS_VARIANT
        # exported, this looked in results/damage_analysis/revision/... instead
        # of .../conus_nandu_v1/..., found nothing, and silently returned 0.
        os.environ["RESIFLOW_RESULTS_VARIANT"] = hz["variant"]
        os.environ["NIRD_RESULTS_VARIANT"] = hz["variant"]
        damage_by_edge, _direct_damage_total = script4.load_event_damage_from_script3(
            base_path, hz["depth_key"], hz["event_key"]
        )
