...
# sctg_summary.py against faf5_od_matrix_by_sctg.pq -- see docs/
# VA_MULTIHAZARD_COMPARISON.md for the caveat: this is a national average
# used as a proxy, not yet the actual commodity mix of each hazard's specific
# disrupted OD pairs. Real per-hazard shares require joining Script 4's
# disrupted-candidate OD pairs against faf5_od_matrix_by_sctg.pq (same
# origin_node/destination_node/Car21 keys as the main assignment) -- not yet
...
def plot_freight_industry_breakdown(
    ...
    *,
    ...
    freight indirect cost -- a proxy, not each hazard's actual disrupted-
    corridor commodity mix (see module docstring above). Pass real per-hazard
    shares (e.g. {hazard_label: {sctg_code: share}}) once that join is wired,
    and set shares_are_national_proxy=False to drop the caveat footnote.
    """
