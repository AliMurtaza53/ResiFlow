...
hazards.conus_multihazard.example.json) rather than hardcoded, so adding/changing a
hazard is a config edit, not a code change -- this is "the one command to
regenerate everything" from docs/CONUS_MULTIHAZARD_METHODOLOGY.md.

Example::

    python scripts/run_conus_multihazard.py \
...
def main() -> int:
    ...
    base_env.setdefault(
        "NIRD_DUCKDB_TEMP_DIRECTORY", str(soge_clusters.parent / "duckdb_tmp_multihazard")
    )
    ...
    for event in events:
        ...
        # wired -- see docs/CONUS_MULTIHAZARD_METHODOLOGY.md "Open: ... freight
