#!/usr/bin/env python3
"""Goal 15 diagnostic: same as run_memleak_repro_500k.py but num_of_cpu=2,
to quantify how much peak RSS during LCP dispatch actually drops when halving
worker count, before committing the full 5M-OD overnight run to num_of_cpu=2.
"""
...
RUN_LABEL = "goal15_memleak_repro_500k_cpu2"
...
del _full_od
print(f"[repro500k_cpu2] Loaded OD subsample: {len(_prebuilt_od):,} rows", file=sys.stderr)


def _patched_load_assignment_demand(base_path, spec=None):
    return DemandLoadResult(
        assignment_od=_prebuilt_od.copy(),
        stats={"source": "prebuilt_500k_repro_cpu2", "combined_flow": float(_prebuilt_od["Car21"].sum())},
    )
...
env = {
    ...
    "NIRD_WORKER_CPU_AFFINITY": "0 1 2 3",
...
print(f"[repro500k_cpu2] label={RUN_LABEL} num_of_cpu=2 max_iterations=3", file=sys.stderr)
print(f"[repro500k_cpu2] PID={os.getpid()}", file=sys.stderr)

os.chdir(REPO_ROOT)
script_path = str(REPO_ROOT / "scripts" / "1_network_flow_model_revision.py")
sys.argv = [script_path, "20", "2"]

t0 = time.perf_counter()
runpy.run_path(script_path, run_name="__main__")
wall = time.perf_counter() - t0
print(f"[repro500k_cpu2] DONE label={RUN_LABEL} wall_sec={wall:.2f}", file=sys.stderr)
